...
fn parsePublicFolder(buf: []u8) ![]const u8 {
    ...
    if (args.next()) |public_folder| {
        return fmt.bufPrint(buf, "{s}", .{public_folder});
    }
...
const Server = struct {
    ...
    pub fn init(self: *Server, options: Options) void {
        log.info("public: {s}", .{options.public_folder});
        zap.enableDebugLog();
        self.* = .{
    ...
    fn handleFallback(r: zap.Request) error{ NotFound, Foobie, MalformedRequestPath }!void {
        ...
        if (mem.lastIndexOfScalar(u8, last_part, '.') == null) {
            ...
            var buf: [fs.max_path_bytes]u8 = undefined;
            const new_path = fmt.bufPrint(&buf, "{s}{s}{s}", .{
                path,
                if (mem.endsWith(u8, path, "/")) "" else "/",
                "index.html",
            }) catch return error.Foobie;
